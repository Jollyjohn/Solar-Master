...
//byte pachube[] = { 173, 203, 98, 29 };    // New Pachube address which doesn't seem to work
//byte pachube[] = { 209, 40, 205, 190 };   // Old Pachube address
//byte pachube[] = { 216, 52, 233, 122 };   // From the COSM examples
//byte pachube[] = { 64,  94,  18, 122 };   // From DNS for api.cosm.com
//byte pachube[] = { 64,  94,  18, 121 };   // From DNS for api.pachube.com
//byte pachube[] = { 192, 168, 100, 25 };     // Local test address for debugging
...
byte dns_srv[] = { 192, 168, 100, 9 };                 // The DNS Server
...
DeviceAddress pool_sensor =   { 0x28, 0xDA, 0x8D, 0xE3, 0x02, 0x00, 0x00, 0xBC };
...
if ( (month < March) || (month > July) ) {
  ...
  */
    if (old_minute != minute) {
      ...
      if (client.connect("api.cosm.com", pachube_port)) {
